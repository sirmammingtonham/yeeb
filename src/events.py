...
with open(paths.ER_WORDS, 'r') as f:
    ihardlyknowher = f.read().split()
...
class Events(commands.Cog):
    ...
    async def _crabby(self, message, text):
        ...
        for frame in ImageSequence.Iterator(im):
            ...
            # draw message
            myFont = ImageFont.truetype(paths.CRAB_FONT, 42)
    ...
    async def on_ready(self):
        self.bot.loop.create_task(change_status(self.bot, headings(
            'List of video games notable for negative reception')))
        print('it seems to be working...')

    @commands.command(name='shut', aliases=['shuttup', 'shuddap'])
    @commands.has_permissions(administrator=True)
    async def shut_up(self, ctx):
        self.enabled = False

    @commands.command(name='speak', aliases=['speakup'])
    @commands.has_permissions(administrator=True)
    async def speak_up(self, ctx):
        self.enabled = True
    ...
    async def on_message(self, message):
        ...
        if message.guild is not None and message.guild.id in self.ignore_servers:
            return
